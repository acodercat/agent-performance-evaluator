...
def get_weather(city: str, date: str) -> dict[str, Any]:
    ...
    Returns:
        ...
        or
        dict: {
            "error": str  # Error message if city or date not found
        }
    Examples:
        >>> get_weather("London", "2023-04-28")
        {
            "city": "London",
            "date": "2023-04-28",
            "data": {"temp": 15, "condition": "Cloudy", "rain_chance": 40}
        }
    """
    ...
    if date not in weather_data[city]:
        return {"error": f"No weather data found for {city} on {date}"}
